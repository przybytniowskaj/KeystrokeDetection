...
def train(cfg: DictConfig):
    ...
    data_loaders, num_classes, class_encoding, batch_size = get_all_dataloaders(
    ...
    optimizer = OPTIMIZERS[cfg.optimizer](parameters, **optimizer_params)
    ...
    if cfg.scheduler == "chained":
        ...
        scheduler = SCHEDULERS[cfg.scheduler](optimizer, **scheduler_params)
    ...
    for epoch in (
        ...
        val_res, _, _ = evaluate_model(device, model, criterion, val_loader)
        ...
        if (
            stop_counter_loss > max_no_improvement
            or stop_counter_acc > max_no_improvement
        ):
    ...
    for name, loader in test_loader.items():
        ...
        try:
            ...
            run.log(
                {
                    f"confusion_matrix_{name}": wandb.Image(
                        image, caption=f"Confusion Matrix: {name}"
                    )
                }
            )
            os.remove(cm_path)
    ...
    for group, datasets in DATASET_GROUPS.items():
        ...
        for dataset in datasets:
            ...
            accs += (
                np.array(test_results[dataset]["res"][1:7])
                * test_results[dataset]["length"]
            )
        ...
        try:
            ...
            run.log(
                {
                    f"confusion_matrix_{group}": wandb.Image(
                        image, caption=f"Confusion Matrix: {group}"
                    )
                }
            )
