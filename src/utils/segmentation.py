...
from constants.segmentation import LABEL_MAP, DATASET_CONFIG
...
def plot_energy(
    ...
    if save_plots:
        plt.savefig(
            os.path.join(output_dir_img, f"{dataset}_{subfolder}_{key.lower()}.png")
        )
    if show_plots:
        plt.show()
    plt.close()
    ...
    if save_plots:
        plt.savefig(
            os.path.join(
                output_dir_img, f"{dataset}_{subfolder}_{key.lower()}_energy.png"
            )
        )
    if show_plots:
        plt.show()
...
def trim_silence_from_signal(signal, min_length=400):
    def trim_leading_silence(x):
        return x[max(0, detect_leading_silence(x) - min_length) :]
...
def save_segmented_strokes(
    strokes, label, output_dir, dataset, subfolder, sample_rate, threshold, min_length
):
    os.makedirs(os.path.join(output_dir, "tresholds"), exist_ok=True)
    with open(
        os.path.join(output_dir, "tresholds", f"{dataset}_tresholds.txt"), "a"
    ) as f:
        f.write(f"{threshold}\n")

    keyboard_type = "mac"
    recording_type = (
        "zoom" if dataset == "practical" and subfolder == "Zoom" else "live"
    )
    ...
    last_skip = None
    if (
        "keystrokes_to_skip" in DATASET_CONFIG[dataset]
        and label in DATASET_CONFIG[dataset]["keystrokes_to_skip"]
    ):
        skip = DATASET_CONFIG[dataset]["keystrokes_to_skip"][label]
    if (
        "last_strokes_to_skip" in DATASET_CONFIG[dataset]
        and label in DATASET_CONFIG[dataset]["last_strokes_to_skip"]
    ):
    ...
    filtered_strokes = [
        ...
        if not (
            (skip is not None and i in skip)
            or (last_skip is not None and i == len(strokes) - 1)
        )
    ]
    ...
    for split_strokes, split_dir in [
        ...
        for i, stroke in split_strokes:
            ...
            signal = AudioSegment(
                stroke_int16.tobytes(),
                frame_rate=sample_rate,
                sample_width=2,
                channels=1,
            )
            signal = trim_silence_from_signal(
                signal, f"{filename_base}_{i}.wav", min_length
            )
...
def process_audio_files(
    audio_file, output_dir, output_dir_img, dataset, save_plots=False, show_plots=False
):
    ...
    steps = (
        DATASET_CONFIG[dataset]["steps"] if "steps" in DATASET_CONFIG[dataset] else None
    )
    thresholds = (
        DATASET_CONFIG[dataset]["thresholds"]
        if "thresholds" in DATASET_CONFIG[dataset]
        else None
    )
    for root, _, files in os.walk(audio_file):
        ...
        for file in tqdm(files, desc=f"Processing files in {subfolder}"):
            if file.endswith(".wav") or file.endswith(".m4a"):
                ...
                if (
                    ...
                    and key in DATASET_CONFIG[dataset]["thresholds"]
                ):
                    threshold = thresholds[key]

                overlap_rate = DATASET_CONFIG[dataset]["overlap_rate"]
                if (
                    "overlap_rates" in DATASET_CONFIG[dataset]
                    and key in DATASET_CONFIG[dataset]["overlap_rates"]
                ):
