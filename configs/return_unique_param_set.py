...
def get_indices_from_task_id(
    list1, list2, list3, list4, list5, list6, list7, list8, task_id
):
    ...
    if task_id >= total_combinations:
        raise ValueError(
            f"task_id {task_id} is out of range for the given lists. Maximum allowed is {total_combinations - 1}."
        )
...
def main():
    ...
    parser.add_argument(
        "-l", "--learning_rates", nargs="+", type=float, help="Learning rates"
    )
    ...
    parser.add_argument(
        "-a", "--architectures", nargs="+", type=str, help="Architectures"
    )
    parser.add_argument(
        "-k", "--special_keys", nargs="+", type=str, help="Special Keys"
    )
    ...
    task_id = args.id
    indices = get_indices_from_task_id(
        learning_rates,
        schedulers,
        optimizers,
        decays,
